...
def test_efficiency_comparison(n_samples=128, grid_dim=180, n_months=36):
    ...
    start = time.time()
    series_pd = pd.Series(pa.Array.to_pandas(arrow_list_array), dtype=pd.ArrowDtype(pa.list_(pa.float32())))
    df_pd = pd.DataFrame({"pred_sb_raw": series_pd})
    pandas_arrow_mem = get_mem() - base_mem
    print(f"State B (Pandas-Arrow): {pandas_arrow_mem:.4f} GB (Created in {time.time()-start:.2f}s) | Tax: {pandas_arrow_mem/actual_data_size_gb:.2f}x")
    ...
    print(f"State C (Polars-Native): {polars_mem:.4f} GB (Created in {time.time()-start:.2f}s) | Tax: {polars_mem/actual_data_size_gb:.2f}x")
