...
# networks
from views_hydranet.architecture.HydraBNrecurrentUnet_06_LSTM4 import HydraBNUNet06_LSTM4
...
def get_data(model_path: ModelPathManager, config):
...
def get_full_tensor(views_vol, config = None):
    ...
    full_tensor = torch.tensor(views_vol.copy()).float().unsqueeze(dim=0).permute(0,1,4,2,3)[:, :, ln_best_sb_idx:last_feature_idx, :, :] 

    # Make a metadata tensor with evrything else
    metadata_tensor = torch.tensor(views_vol.copy()).float().unsqueeze(dim=0).permute(0,1,4,2,3)[:, :, :ln_best_sb_idx, :, :]
