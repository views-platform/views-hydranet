...
class IntegrityGuardian:
    ...
    def monitor(
        ...
        if not torch.isfinite(prediction).all() or prediction.abs().max() > 10000:
            ...
            error_msg = f"[FATAL NUMERICAL EXPLOSION] Predictions exploded (Max Abs: {p_max:.2f}) at {context}"
            logger.error(error_msg)
            raise RuntimeError(error_msg)
        ...
        for name, param in model.named_parameters():
            if param.grad is not None:
                if not torch.isfinite(param.grad).all():
                    error_msg = f"[FATAL GRADIENT EXPLOSION] NaN/Inf detected in gradients of {name} at {context}"
                    logger.error(error_msg)
                    raise RuntimeError(error_msg)
    ...
    def check_weights(model: nn.Module, context: str = "") -> None:
        ...
        for name, param in model.named_parameters():
            if not torch.isfinite(param).all():
                error_msg = f"[FATAL WEIGHT CORRUPTION] NaN/Inf detected in weights of {name} at {context}"
