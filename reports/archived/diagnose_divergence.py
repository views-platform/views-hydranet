...
def main() -> None:
    ...
    if p2.get("subject_maxima_differ"):
        ...
        print("  Neither backend is wrong — these are legitimate UCDP revision differences.")
        print()
        ...
        print()
        print("  The prediction divergence is a structural property of the training")
        print("  pipeline. The curriculum's hard threshold (activity >= threshold)")
        print("  creates discrete jumps in window selection from tiny perturbations.")
        print("  The same divergence would occur from:")
        print("    - The same source fetched at different UCDP release dates")
        print("    - Slightly different calibration partition boundaries")
        print("    - A single cell-month value changed by any amount")
        print()
        print("  The training process is effectively chaotic: deterministic given")
        print("  identical inputs, but sensitive to arbitrarily small perturbations.")
        print()
        print("  IMPLICATIONS:")
        print("  1. A single training run samples one trajectory through a chaotic")
        print("     loss landscape. Predictions from any single run should not be")
        print("     treated as point estimates.")
        print("  2. Consider ensemble averaging over multiple runs (different seeds")
        print("     or slight data perturbations) for more stable predictions.")
        print("  3. Consider softening the VolumeSampler threshold boundary")
        print("     (e.g., probabilistic weighting instead of hard cutoff) to")
        print("     reduce sensitivity to small data changes.")
