...
| **M56** | **Freezing semantics at n=4: the CELL carries it, `all` adds nothing, and nothing hurts.** L=300, `sb`, **4 seeds (42/43/44/45)**, emit only, 13 origins, 16 arms unattended in 167 min, **zero failures**. One flag: `freeze_recurrent` ∈ {none, hidden, cell, all}. **AP@h36 vs none: cell +0.0591 (4/4, sd 0.0067) · all +0.0578 (4/4) · hidden +0.0186 (4/4)** — `all` and `cell` are indistinguishable inside the seed spread, so **freezing both halves buys nothing over freezing the cell** and M39's 89% attribution is confirmed at n=4; its pre-registered falsifier (all > cell beyond spread) did not fire. `hidden` alone is real at long horizon but **~3× smaller**, and **CONTESTED at h18** (2 of 4 seeds negative). **B.3: no arm is consistently worse than none on any head at any horizon** — the clamp is strictly non-harmful here, which `05` pre-committed to treating as a finding rather than an absence. **Replicates M48** (+0.059 at h36) on an independent re-run with a changed dump instrument. | drivers EXP Wave 1 | **High** — n=4, pre-registration locked at `0a76a6a` before the first arm, decision rule reused from M48 (4/4 sign agreement AND |mean| > seed sd) with **no p-value claimed** because a paired sign-flip at n=4 floors at 1/16 = 0.0625. Both reproduction falsifiers EXACT to the last digit; every dump carries `n_passes = 4`; 13/13 origins per arm enforced. ⚠️ `hs = o ⊙ tanh(cl)` (C-292) makes the hidden/cell split the caveated half — freezing one while the other evolves builds a state pair the model would never produce. ⚠️ **`sb` only — LIFTED by M59** (holds on `ns`/`os` too, 4/4). **Cost: 167 min, emit only.** |
| **M57** | **The clamp is NOT a continuation-only trick — it improves ONSET on 4/4 seeds.** Same wave. Cells split by their state at the ORIGIN month into two well-posed ranking problems: **continuation** (already active — do they persist?) and **onset** (quiet — does conflict START here?). `cell` minus none, **all four SUPPORTED at 4/4 seeds**: continuation **+0.0437** (h18) / **+0.0614** (h36); onset **+0.0224** (h18) / **+0.0459** (h36). Onset is ~**38% relative** at h36 against a base rate of 0.0079, and the unclamped model already has real onset skill (**15×** chance). **This was the only question in the battery that could have changed the ship decision** — a headline AP gain made entirely of conflict that was already there would be close to worthless for the product, and *no statistic measured before this dossier could tell the difference* (**C-319**). It is not that. | drivers EXP Wave 1 | **High for the direction, qualified on the ranking** — 4/4 seeds in both universes at both horizons. ⚠️ **The pre-registered falsifier was AMBIGUOUS and is recorded as a defect, not repaired post hoc**: it said the gain on new cells must be "at least as large" without naming the measure, and absolute gain and skill-above-base favour continuation while relative gain favours onset. All three are reported; none was chosen after the fact. **Third mis-specified gate in this programme** (after F3's unsatisfiable band and FR-4's untested assumption), same root cause each time: a threshold written without first checking what the statistic can deliver. ⚠️ Base rates differ ~40× between universes, so the two APs are comparable **across arms only**, never to each other. **`sb` only — LIFTED by M59** (holds on `ns`/`os` too, 4/4). |
| **M58** | **Freezing flattens what the model says about CHANGE, and costs no skill at saying WHICH places change — and it corrects M55 on the body.** Same wave. Cohort fixed at h1 **by truth** (n=1343), so no arm can move its own denominator (**C-319**). **Q0.1 gates the rest:** direction skill is weak but real and *rises* with horizon (rho ≈ 0.14 at h36), clearing the 0.05 unanswerable-threshold from h18 — so C.1–C.3 are answerable at long horizon and **marginal at h6–h12**, reported as such rather than as nulls. **C.2: dispersion of the predicted per-cell change falls −0.4563 (h18) and −0.6593 (h36), 4/4 seeds**, under the CELL clamp; freezing *hidden* has no consistent effect (contested at both) — the M39 asymmetry now visible in the dynamics. **C.3: it costs nothing** — rho is *slightly better* under the clamp at h18 (+0.0137, 4/4) and indistinguishable at h36. The trade-off the question was built to catch does not exist. ⛔ **CORRECTS M55 and my own seed-42 interim:** I wrote the clamp does "nothing" for the body. Too strong. `crps_events` improves on **4/4 seeds in every arm** (`cell` −6.7/−6.1/−6.5/−4.1 % at h18), but fades to **−0.6%** by h36 — while `size_ratio` is **exactly 0 in all 16 arm-seeds**. Accurate: **a small, replicated gain in the body's proper score and no movement in its calibration at all.** | drivers EXP Wave 1 | **High for the dynamics, honest about the level** — 4/4 on every SUPPORTED line. ⚠️ **rho ≈ 0.14 is weak skill absolutely, so "no cost" partly reflects that there was little to lose.** ⚠️ The dispersion measure has a documented EPS floor (~0.002 for a uniform rescale) and must be read as "materially above the floor", never as "non-zero". ⚠️ `sb` only — **LIFTED by M59**; hidden/cell split carries the C-292 caveat. |
| **M59** | **It all holds on `ns` and `os` too — and the dynamics trade-off is BETTER there.** **Zero GPU**: the body-mean dumps carry all three targets (`mu` is `[36,3,180,180]`); only the score CSVs were sb-only. Pure re-analysis of Wave 1, same mutation-tested instruments, and `sb` reproduces identically after the refactor so it is provably inert on the established results. **C.4: every one of twelve lines SUPPORTED at 4/4** — the cell clamp improves BOTH onset and continuation on all three targets at both horizons, and its continuation effect is **2.3–2.6× LARGER on `ns`/`os`** than on `sb` (h36 cont: sb +0.0614 · ns +0.1401 · os +0.1582). **C.3 flips from neutral to positive:** rho at h36 is CONTESTED on `sb` (−0.0020) but **SUPPORTED 4/4 on `ns` (+0.0177) and `os` (+0.0598)** — so on two of three targets the clamp compresses the predicted amplitude of change **and improves the ordering of which places worsen**, which is better than the "no cost" `sb` showed. **C.2 and cell-over-hidden generalise**: dispersion falls 4/4 under `cell` on every target, `hidden` flattens `ns`/`os` too but always less (−0.34 vs −0.90 on `ns`), and cell beats hidden on onset at h36 on all three (margin narrowest on `os`). **Lifts the "`sb` only" caveat from M56/M57/M58.** | drivers EXP D.2 | **High** — n=4 seeds × 3 targets, same decision rule, no new code path in inference and no new GPU. ⚠️ Not independent of Wave 1: the same 16 arms, re-read on different output channels, so this is **replication across TARGETS, not across runs**. ⚠️ The scorer's AP/Brier/`crps_events`/`size_ratio` remain sb-only, so B.1's body result is **not** extended here — only the dump-derived measures are. **Cost: zero GPU.** |
| **M60** | **The CELL STATE drives where the model points; the fed-back input does not.** Wave 2, 6 arms (cell/input/hidden × seeds 42,43), 13 origins, 78 min, 6/6 OK. Roll ONE live driver 90 cells at **every** step, then ask via the EXP-3b cross-correlation instrument whether the emitted gate field followed it. **Fraction of the 26 origin-seed pairs that followed: cell 26/26 at h2, h4, h6 AND h12 · input 0/26 at EVERY horizon · hidden 0/26.** The input-rolled arm stays **0.79–0.93 correlated with the unrolled baseline at zero offset** — displacing what the model is fed barely perturbs the emitted field at all. **Answers A.1/A.2 directly** where Wave 1 could only answer by intervention, and adds what Wave 1 could not: the input contributes essentially nothing to the current step's placement. **Not a contradiction of `use_real`** (M51: occurrence ×1.19 vs ×0.036) — the input acts **through the state across steps**, not on the present step's placement, which is precisely the feedback loop this programme has been chasing. With **M54** (the state is a map) and **M56** (cell ≫ hidden) the chain is: the cell state holds a learned spatial prior, free-running drains it, and clamping hands it back. | drivers EXP Wave 2 | **High for h2–h12, not established beyond** — 26/26 is unanimous across origins and seeds at four horizons; the decay to 16/26 (h24) and 9/26 (h36) is the rolled pattern losing coherence through repeated rolling and the network's non-linearity. ⛔ **Records a correction to my own reading:** from ONE origin mid-run I saw (0,0) at h18/h36 and concluded the 90-shift was **aliasing** (90+90=180≡0 on a 180 grid). Wrong — 26/26 follow at h2/h4/h6/h12, whose roll counts differ in parity, which no aliasing rule produces. **A single origin is not a measurement**, and I drew a mechanism from n=1 when the instrument pools 13. ⚠️ Seeds 42/43 only; one shift (90) — a shift coprime with 180 would separate coherence-decay from residual aliasing and is worth running. ⚠️ Gate field, `sb`. Instrument: 14 tests, **8/8 mutations caught**, one of which found that rolling the input on the CHANNEL axis survived every other test and would have inverted the verdict. **Cost: 78 min, emit only.** | ⚠️ **QUALIFIED 2026-09-03 by reading the architecture.** "hidden 0/26" does **not** mean the hidden state is inert or broken. `HydraBNUNet06_LSTM4.forward` is a textbook ConvLSTM: `hs = o_t * tanh(hl)` — the hidden half is **recomputed from the cell every step**. So a displaced `hs` is regenerated from the *undisplaced* cell at the next step: the perturbation **self-heals**, and the roll test is architecturally guaranteed to return 0 regardless of how much work hidden does. What the 0/26 actually establishes is narrower and still useful: **hidden carries no spatial information ACROSS steps**, so it cannot be the thing that drains. Hidden is doing real work — it drives all four gates and is concatenated onto the encoder input (`x = cat([x, hs_1..hs_4])`), so it shapes what the U-Net sees — it simply has no memory of its own. Only the cell accumulates (`hl = f*hl + i*hl_tilde`). By the same reasoning, **M56's `hidden` freeze arm is an architecturally abnormal operation**: it feeds a stale `hs` to the gates and encoder while the cell evolves, breaking the LSTM's own recurrence, which makes its +0.019 hard to interpret as "what hidden contributes".
| **M61** | **Connecting the gradient through the sampled feedback (BPTT-SA, straight-through) makes training diverge; cutting it does not.** Single variable, ε=0.5, seed 42, same data, same code, `nb` family, 300 lessons pre-registered. **Arm A (wire cut) trained all 300 lessons in 184 min and produced an artifact. Arm B (wire connected) died at lesson 48 with `[FATAL GRADIENT EXPLOSION] NaN/Inf in enc_conv0.weight`** — raised by the `IntegrityGuardian`, with gradient clipping ON in that config. **The hypothesis is still NOT TESTED**, for the second time: the pre-registered rule needs AP@h18 from both arms and arm B produced no artifact. **Contrast with the 2026-06-10 SS bracket** (0.25→explode, 0.5→explode, 1.0→collapse): that was the hurdle/lognormal head, and plain SS at ε=0.5 is now **stable for 300 lessons** on the `nb` head — so this is not the old scheduled-sampling explosion resurfacing; the gradient wire is what broke it. | #308 SCREEN-2, `reports/2026-09-03_bptt_sa_dossier/07` | **High that the flag causes it** (clean single-variable contrast, loud failure, and the potency gate passed at launch — `off=0, on=66.63` — so the knob was verifiably live, unlike SCREEN-1). **The MECHANISM is UNKNOWN and is recorded as unknown.** Three candidates tested, none reproduces it: gradient norm vs sequence length on a toy head (ratio 0.999–1.000 at 1…31 steps), the same on the **real** `HydraBNUNet06_LSTM4` at 31 steps (1.001, all finite), and surrogate overflow (both paths finite with `mu` swept to 99). The extra path is ~0.1% of the gradient norm at init ⇒ an **emergent training-dynamics** instability, not an initialisation-time or overflow one. σ_max ≈ 7.76 (#294) predicts an exploding Jacobian product and is **consistent but not evidence**; the two static tests are mildly against it. ⛔ **Scope, per amendment A1 committed before the result:** the straight-through estimator is **biased**, so the supported claim is *"this straight-through implementation is unstable here"*, **NOT** *"BPTT-SA is unstable here"* and **NOT** *"BPTT-SA does not help"*. An unbiased reparameterised feedback (#308 follow-up) is untested. **By-product:** arm A is a fresh SS arm on today's code, which removes the August-baseline drift confound SCREEN-1 carried. ⚡ **MECHANISM FAMILY IDENTIFIED 2026-09-04 by the GRAD-TRAJ probe (36 min, rule committed before the run): CREEP, not JUMP.** The pre-clip gradient norm of the attached arm rises **133,465 → 9.4×10⁹** between lessons 15–25 and 38–47 (**70,468×**, Spearman ρ=+0.561) while the control's **falls** 859 → 312 (ρ=**−0.764**) — `attached/control = 3.0×10⁷` at the late window, and already **155×** apart by the early one, i.e. from the moment ε saturates at lesson 15. It compounds for 33 lessons until float32 gives out (7.6×10¹⁸ squared = 5.8×10³⁷ vs a 3.4×10³⁸ ceiling). **Deterministic** — lesson 48 again on a fresh clone. ⛔ **The loss is blind to it:** `loss_reg` FALLS 655 → 255 and the gate logit drifts smoothly across the same 48 lessons, so every curve a human watches says the run is training well. Clipping is on and bounds the *step*; the *intermediate* gradient is what overflows, so a fix must bound the gradient **per step inside the feedback wire**. ⛔ **CORRECTS this entry's own "three candidates tested, none reproduces it":** two of those three measured a network at **initialisation**, where the extra path really is ~0.1% of the norm — an untrained network cannot show an instability that training builds (**C-325**). σ_max≈7.76 (#294) is now consistent and still not established: CREEP names a family, not a cause. A stabiliser is now worth GPU; it was not before this measurement. ✅ **STABILISED 2026-09-04, same day:** a per-step bound on the feedback gradient (`ss_feedback_grad_clip=1.0`, 9/9 mutations caught) runs **80 lessons clean** with the gradient back **on the control's scale** (913 vs 859 at L15–25) and trending DOWN (ρ=−0.306, late/early 0.914× vs 70,468×); peak 13,021 vs 7.6×10¹⁸. Fixing it by bounding exactly the quantity GRAD-TRAJ measured **independently confirms CREEP** — a JUMP-class arithmetic defect would not yield to a gradient bound. ⚠️ **Amendment A2, pre-registered:** the clip bites on every step (pre-clip `fed` norm median 5.7–17.5 vs a threshold of 1.0), so a NULL on the screen may NOT be read as "BPTT-SA does not help" — it obliges a threshold ladder first. A positive result is unaffected. **H remains untested.** |
| **M62** | **BPTT-SA via a straight-through estimator HURTS, and it hurts by making the model FIRE MORE.** SCREEN-3, the first time #308's question actually got answered: both arms trained 300 lessons, weight hashes differ (not void), neither crashed (BRANCH 0 not triggered). **`AP@h18` (sb, free-running, 13 origins): control 0.3064 → treated 0.1997, Δ = −0.1066** — a 35% relative loss, **5×** the pre-registered +0.02 threshold and ~1.8× the ~20% training variance this n=1 design is limited by. Worse at every horizon (h1 −0.072, h18 −0.107, h36 −0.090). **The mechanism is occurrence, not magnitude:** `act_ratio` h18 **0.036 → 0.107 (×3.0)**, h36 **0.0026 → 0.0249 (×9.6)**, while `size_ratio` stays 0.000 in both and `crps_events` is unchanged. **This is M45 again** — "AP loss scales with how much the model FIRES" — a fifth intervention landing on the same lever and falling off it the same way. Vehicle sane: the control's 0.3064 sits between the untouched L=300 reference 0.3318 (M34–M37) and the ≈0.289 that M30–M33's −0.0426 SS penalty predicts. | #308 SCREEN-3, `reports/2026-09-03_bptt_sa_dossier/07` | **High for the measurement, and it partly resolves its OWN confound.** Amendment A2 (pre-committed) said a null obliges a clip ladder because the clip attenuates the feedback gradient ~10×. **The data argues against A2 directionally:** attenuating toward zero makes the treated arm *approach* the control — with no signal on the wire it IS plain scheduled sampling — so throttling predicts **convergence**, and what was measured is a 35% **divergence**. Attenuation manufactures sameness, not harm of this size. **A1 survives and is now the prime suspect:** a biased estimator does not weaken the signal, it points it the WRONG WAY, and more wrong direction is actively harmful — the straight-through surrogate substitutes the composed MEAN's gradient for the DRAW's. ⛔ **Therefore this is NOT "BPTT-SA does not work".** The honest reading is that **the straight-through approximation is the leading suspect for the harm, and the idea underneath it is still untested** — an unbiased reparameterised feedback is the open question. The ladder changes purpose from rescue to a DISCRIMINATING test with opposed predictions: under A1 a higher clip is worse or unstable, under A2 it is better. ⚠️ n=1, one seed, one configuration; a SCREEN. |
| **M63** | **The straight-through estimator is NOT backwards — so BPTT-SA's harm is the OBJECTIVE's, not my approximation's.** EXP-BIAS measured what the estimator ADDS to the gradient (`Δ_ST`) against the unbiased score-function term it SHOULD add (`Δ_SF`), both averaged over the same draws, on both trained 300-lesson artifacts, on their own real data, n=1024 draws × 3 handoffs. **`cos(Δ_ST, Δ_SF)` on the arm trained WITH the wire: +0.882 / +0.765 / +0.756**; control +0.213 / +0.707 / +0.782. Pre-registered threshold was ≥ +0.3 ⇒ **A1 refuted, A3 leads**. So the −0.1066 AP@h18 of **M62** is not my approximation pointing the wrong way: **BPTT-SA on this problem genuinely trains the model to fire more**, and firing is the lever M32/M45 and three other interventions already closed. A fifth confirmation of that finding, not a new failure. | #308 EXP-BIAS Phase 1, `reports/2026-09-03_bptt_sa_dossier/07` | **High.** Instrument gates all passed: split-half reliability of the reference **0.79–0.99** with parity and permuted splits agreeing to ~0.05; negative control ≤ 0.0018; forward identity `L_cut == L_on` to ≤ 5.5e-07; and two EXACT-TRUTH controls against closed-form arithmetic (Bernoulli `dE/dp = f(1)−f(0)`, Gaussian `dE[z²]/dμ = 2μ`). Instrument: 18 tests, **16/16 mutations caught**; two real bugs were caught by those tests BEFORE any data — a post-mask count scored against the nb density (the very substitution error under investigation) and a Bernoulli mask drawn channel-FIRST where production draws channel-LAST, which gives a different mask from the same seed. ⚠️ **Plateau gate formally met at only 3 of 6 handoffs**, but its failure direction is known: cos RISES with N because a noisy reference can only attenuate a correlation toward zero, so +0.76 is a **lower bound**. ⛔ **Corrects my own n=256 reading**, where the control's split-half came back −0.06/−0.17 and I suspected my even/odd split; a permuted split at n=1024 agrees with parity — it was sample size. ⛔ **Scope: "not backwards", NOT "unbiased"** — cos≈0.8 leaves ~0.6 rad of misalignment, and an unbiased reparameterised feedback remains untested. One handoff, one window, one seed per arm. |
| **M64** | **Input-noise augmentation (occurrence dropout, p=0.204) is substantially HARMFUL, and it fails on the firing lever — the sixth time.** Epic #311, 2 arms × 300 lessons, ε=0, seed 42, one variable, both retrained. **`AP@h18` (sb, free-running, 13 origins): control 0.3292 → noise 0.1329, Δ = −0.1963** — a 60% relative loss and **3.3×** the ±0.06 band this n=1 design cannot see inside; worse at every horizon. **The mechanism is occurrence:** `act_ratio` at h18 **0.0123 → 0.6903 (×56)**, h36 ×1,180, `Brier` worse everywhere, `crps_events` barely moving. **M45 for the sixth time.** ⛔ **And the reason is a DESIGN error worth keeping:** the augmentation deletes events from the INPUT while the TARGET keeps them, so the task becomes *"given a field missing a fifth of its events, predict one that still has them all"* — whose optimal response is **to invent occurrence**. The "no target adjustment" step came from `SanchezGonzalez2020`'s own supplement (*"happens implicitly when the loss is on next-step ground-truth"*), which is true for their **small symmetric jitter on a dense field** — zero-mean, so the implicit target is unbiased — and **does not survive the change to deletion**, which is not zero-mean. S1 measured correctly and the rule picked the right FAMILY; the error was the step from *"the model silences events"* to *"so silence events in its input"*, which mimics the **symptom** the model produces rather than the **conditions** it faces. | #311 S5/S6, `reports/2026-09-04_input_noise_dossier/07` | **High for this design.** All gates passed before the number was read: smoke; potency on the arm's own config at a **trained** checkpoint (loss 265.80 → 106.27, rel 0.600); **FG-A PASS** and FG-C PASS (C-299, the first dossier to invoke `floor_gate` since August); weight hashes distinct. Vehicle sane — control 0.3292 vs the known ε=0 reference 0.3318 (M34–M37). ⛔ **Scope, and the locked rule forbids more:** this is NOT a claim about input-noise augmentation in general, other rates, or a variant that degrades the TARGET alongside the input. One design, one rate, one seed, one vehicle. A 4-seed run is **not** worth buying — the pre-registered trigger for seeds was Δ ≥ +0.02, and measuring −0.196 more precisely buys nothing. ⚠️ **QUALIFIED 2026-09-05 by `/code-review max`: the arm ran a HARSHER schedule than the design specified.** The implementation dropped on the first step of every segment as well, but the design fits survival as `(1-p)^(h-1)` — horizon 1 **clean** — and deployment matches it (inference feeds the real observation at the seed step). Implemented vs fitted survival: h1 **0.796 vs 1.0**, h18 0.01646 vs 0.02068, h36 0.00027 vs 0.00034 — a flat **20.4% over-silencing at every horizon**, four times the design's own 5% residual tolerance. **The direction is unaffected** (the mechanism is over-firing caused by intact targets, which the schedule does not change) **but the effect size is not attributable to p=0.204 as designed.** Fixed in code; a re-run at the corrected schedule was NOT bought, because the pre-registered trigger for more GPU was Δ ≥ +0.02 and the measured Δ is −0.196. |
| **M65** | **The seed variance this programme designs against is ~5× too large, and a PAIRED 2-seed design detects effects 3× smaller than our best result.** Measured from artifacts already on disk — **zero GPU** — on the clean 4-seed same-config control set (`fullzero_forty{two,three,four,five}_identity`, L=300, `sb`, free-running, 13 origins). **Unpaired seed sd: h1 0.0035 (0.7%), h18 0.0134 (4.1%), h36 0.0122 (5.4%)** — not the **~20%** taken from C-119/C-184 and used to size every recent screen. **Paired within seed (control vs `freezecell`, same seed both arms): sd h18 0.0075, h36 0.0067.** Design consequences, one-sided α=0.05 at 80% power: unpaired n=1 gives MDE **0.0473** at h18, which is **larger than the +0.0367 the cell freeze actually produces** — so every n=1 screen this programme has run was blind to an effect the size of its own best result, exactly as the method-review panel argued. **Paired at n=2 gives MDE 0.0131**, i.e. **2.8× smaller than the freeze effect.** Doubling the GPU budget from ~8 h to ~16 h moves the design from "can only detect a disaster" to "can detect the effect that matters". **By-product — the cell freeze re-verified from the raw CSVs**: ΔAP h18 **+0.0367**, h36 **+0.0591**, **4/4 seeds positive** at both, and **exactly +0.0000 at h1 on all four seeds** (byte-identical), confirming it is a pure rollout intervention with no short-horizon cost. | recomputed 2026-09-05 from `reports/2026-09-03_drivers_and_dynamics_dossier/**/score_fullzero_*_identity*.csv` | **High for the arithmetic, and it is only arithmetic** — four seeds, one config, one vehicle, one target. ⚠️ **This does NOT retire C-119.** The ~20% figure came from the seed-bimodal BatchNorm era before the C-184 recalibration mitigation shipped; what is measured here is the *post-mitigation* vehicle at L=300. The honest claim is "the variance that applies to screens run on today's vehicle is ~4%", not "C-119 was wrong". ⚠️ n=4 gives a sd estimate with wide uncertainty of its own; treat 0.0134 as an estimate, not a constant. ⚠️ The paired sd is measured on an **emit-only** intervention where both arms share a trained model — a *training-time* treatment cannot cancel the seed draw the same way, so the paired MDE above is a **floor**, not a guarantee. |
