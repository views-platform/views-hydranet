...
def run_hostile_consumer_audit():
    ...
    df = pd.DataFrame({"pred_sb_raw": df_pl["pred_sb_raw"].to_list()})
    ...
    try:
        ...
        results["B_Apply_Lambda_Mean"] = len(means) == n_rows and isinstance(
            means.iloc[0], (float, np.float32, np.float64)
        )
    except Exception as e:
    ...
    if all_passed:
        ...
        print(
            "The Arrow-backed Pandas DataFrame behaves EXACTLY like a list-in-cell DataFrame to the consumer."
        )
    else:
