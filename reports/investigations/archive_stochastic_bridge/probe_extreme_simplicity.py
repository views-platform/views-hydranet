...
def test_minimalist_loop(n_samples=128, grid_dim=180, n_months=36):
    print("--- Researching Minimalist Loop (NumPy .tolist()) ---")
    base_mem = get_mem()
    ...
    idx = pd.MultiIndex.from_product(
        [range(n_months), range(grid_dim * grid_dim)], names=["m", "p"]
    )
    df = pd.DataFrame(index=idx)
    print(
        f"Pandas Scaffold Created: {time.time() - start:.4f}s | Mem: {get_mem() - base_mem:.4f} GB"
    )
    ...
    for c in range(n_cols):
        ...
        print(
            f"  Column {c} added in {time.time() - start_col:.4f}s | Peak RAM: {get_mem() - base_mem:.4f} GB"
        )
