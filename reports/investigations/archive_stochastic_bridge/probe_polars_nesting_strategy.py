...
def test_exploded_nesting_strategy(n_samples=128, grid_dim=180, n_months=36):
    ...
    pg_ids = np.tile(np.repeat(np.arange(grid_dim * grid_dim, dtype=np.int32), n_samples), n_months)
    ...
    df_pd = pd.DataFrame({
        "pred_list": df_nested["pred_list"].to_list()
    })
    print(f"Pandas Conversion (to_list): {time.time()-start:.4f}s | RAM: {get_mem()-base_mem:.4f} GB")
