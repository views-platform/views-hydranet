...
_INTENDED = {"ss_epsilon_max", "ss_feedback", "torch_seed", "np_seed", "total_lessons"}
...
def arm_label(*, lessons: int, eps: float, seed: int) -> str:
    ...
    return f"{_LESSON_WORD[lessons]}{_EPS_WORD[eps]}_{_SEED_WORD[seed]}"
...
def build(*, lessons: int, eps: float, seed: int, out_root: Path = _MODELS) -> Path:
    ...
    if marker not in qtext:
        raise SystemExit("make_ss_arm: config_queryset.py no longer derives model_name as expected")
    qs.write_text(
        qtext.replace(
            marker,
