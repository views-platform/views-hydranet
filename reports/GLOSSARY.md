...
| **retention** | how much of its own h=1 skill a model still has at a later horizon: **AP(h) ÷ AP(h=1)**, free-running, same cube. Locked 2026-08-18; in force since the SS pre-registration, which makes absolute **AP(h18)** primary and retention **co-primary** — the two must agree in sign, because a ratio of two noisy quantities can move on its denominator. Interval: `scripts/ap_block_bootstrap.py --ratio` (paired origin draws). ⚠️ **R > 1 is not a bug** — prevalence rises with horizon (1343 events at h1, 1547 at h18), so AP can legitimately be higher later; the measured oracle sits at 1.0101. | retention rate, skill retention, decay ratio |
| **the ceiling** | the AP the same model reaches at the same horizon when fed **the real field** each step — i.e. the oracle's score. Measured 0.479 at h18 on the 160-lesson reference, against 0.257 free-running. It is the ceiling *of the feedback path*, not of the model: it says what perfect occurrence **and** magnitude would buy, nothing about what is learnable. | headroom, upper bound, oracle ceiling |
...
| **free-running** | the rollout feeding on its own output — `rollout_feedback` ∈ {`mean`, `sample`}, the `identity` arm, the scored product. The thing every rollout number describes unless it says **the oracle**. | autoregressive, self-feeding, closed-loop |
| **the oracle** | the same rollout with `rollout_feedback='teacher_forced'` — the realized truth fed back each step, i.e. the `use_real` arm. Diagnostic only, never a forecast. Its score is **the ceiling**. | use_real, teacher-forced run, perfect-feedback |
