...
1.  **Dumb Mechanic Law:** The Sampler never "decides" what is a good sample. It only filters based on the `Lesson` provided by the Planner (`CurriculumLearner`).
2.  **Custodian Fidelity:** Every extracted window remains a fully functional `VolumeHandler`. Data and Ledger are never decoupled.
...
`CurriculumLearner` → **`VolumeSampler`** → `List[VolumeHandler (Local Windows)]` → `Trainer`.

---

## 4. Contractual Precision (The "Constraints")

### `get_batch(target_name: str, threshold: int, batch_size: int) -> List[VolumeHandler]`
*   **Pre-condition:** `target_name` must exist in the Ledger.
*   **Post-condition:** Returns a list of bit-perfect spatial windows anchored to the correct geographic offsets.
