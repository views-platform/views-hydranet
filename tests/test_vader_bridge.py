...
PHYSICS_CFG = {
    ...
    "derivations": {"binary": [{"from": "lr_feature_a", "to": "by_feature_a", "threshold": 0}]},
}
...
def test_bridge_point_accuracy():
    ...
    pred_handler = handler.wrap_predictions(
        posterior, target_names=["lr_feature_a", "by_feature_a"]
    )
...
def test_bridge_stochastic_accuracy():
    ...
    df_in = pd.DataFrame(
        {"month_id": [1], "priogrid_gid": [1], "row": [0], "col": [0], "lr_feature_a": [10.0]}
    )
    handler = VolumeHandler.from_df(df_in, PHYSICS_CFG)
    ...
    assert val == [1.0, 2.0, 3.0], (
        ...
        "Sample values must be preserved exactly; check collapse_to_point() "
        "and to_evaluation_df()."
    )
...
def test_bridge_contract_violation():
    ...
    df_in = pd.DataFrame(
        {"month_id": [1], "priogrid_gid": [1], "row": [0], "col": [0], "lr_feature_a": [1.0]}
    )
    handler = VolumeHandler.from_df(df_in, PHYSICS_CFG)
    ...
    with pytest.raises(
        ...
        handler.wrap_predictions(posterior, target_names=["lr_feature_a", "by_feature_a"])
...
def test_bridge_naming_suffixes():
    ...
    assert "pred_lr_feature_a" in df_out.columns, (
        ...
        f"wrap_predictions() must prepend 'pred_' to target names."
    )
