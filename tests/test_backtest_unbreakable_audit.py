    ...
    def test_green_temporal_continuity(self, audit_env):
        ...
        with patch(
            ...
            for i, origin_idx in enumerate(origins):
                ...
                expected_start = (
                    100 + origin_idx + 1
                )  # Month at index origin is 100+origin. Forecast starts at +1.

                assert start_month == expected_start, f"DF[{i}] start month mismatch!"

                if i > 0:
                    prev_start = results[i - 1].index.get_level_values("month_id").min()
                    assert start_month == prev_start + 1, (
                        f"Temporal drift detected between DF[{i - 1}] and DF[{i}]!"
                    )
    ...
    def test_green_12_feature_schema(self, audit_env):
        ...
        with patch(
            ...
            expected_features = [
                "lr_sb_best",
                "lr_ns_best",
                "lr_os_best",  # Linear Actuals
                "by_sb_best",
                "by_ns_best",
                "by_os_best",  # Binary Actuals
                "pred_lr_sb_best",
                "pred_lr_ns_best",
                "pred_lr_os_best",  # Linear Predictions
                "pred_by_sb_best",
                "pred_by_ns_best",
                "pred_by_os_best",  # Binary Predictions
            ]
