...
def test_end_to_end_survival_physics():
    ...
    config = {
        "transformations": {"log1p": ["lr_sb"], "asinh": ["lr_ns"], "identity": ["lr_os"]},
    ...
    # Must provide names for all 6 channels in the data
    target_names = ["lr_sb", "lr_ns", "lr_os", "by_sb", "by_ns", "by_os"]
    vh = vh_scaffold.wrap_predictions(data, target_names=target_names)
