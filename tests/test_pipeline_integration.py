...
class TestPipelineIntegration:
    ...
    def test_evaluate_model_artifact_dataframe_track(
        self,
        mock_fmm_init,
        mock_config_init,
        mock_fetcher_cls,
        ...
        """
        prediction_format='dataframe' → _evaluate_model_artifact() must return
        list[pd.DataFrame] (pipeline legacy contract).
        """
        df_config = {**TOY_CONFIG, "prediction_format": "dataframe"}
        mock_fetcher_instance = mock_fetcher_cls.return_value
        mock_fetcher_instance.fetch_df.return_value = toy_dataframe
        mock_fetcher_cls.standardize_raw_df.side_effect = lambda df, cfg: df
        mock_config_init.return_value.get_config.return_value = df_config
        ...
        with patch(
            ...
            mock_configs_prop.return_value = df_config
            ...
            with (
                ...
                for col in ["pred_lr_sb_best", "pred_lr_ns_best", "pred_lr_os_best",
                ...
                result = manager._evaluate_model_artifact(eval_type="calibration")

                assert isinstance(result, list), (
                    f"prediction_format='dataframe' must return list, got {type(result)}"
                )
                assert len(result) == 1
    ...
    def test_forecast_model_artifact_dataframe_track(
        self,
        mock_fmm_init,
        mock_config_init,
        mock_fetcher_cls,
        ...
        """
        prediction_format='dataframe' → _forecast_model_artifact() must return
        pd.DataFrame (pipeline legacy contract).
        """
        df_config = {**TOY_CONFIG, "prediction_format": "dataframe"}
        mock_fetcher_instance = mock_fetcher_cls.return_value
        mock_fetcher_instance.fetch_df.return_value = toy_dataframe
        ...
        with patch(
            ...
            with (
                ...
                assert isinstance(result, pd.DataFrame), (
                    f"prediction_format='dataframe' must return pd.DataFrame, got {type(result)}"
                )
