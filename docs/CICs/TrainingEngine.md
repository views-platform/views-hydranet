...
- **The BN-recalibration passes see CLEAN data (invariant):** both `_recalibrate_bn` and a `bn_recal_from` run pass `training_augmentation=False`, which suppresses **every** training-only augmentation on those passes — the input noise, `random_flips`, **dropout** and **scheduled sampling** alike. ⚠️ The first three words of that sentence were **false until 2026-09-14 (Epic #353 / S3 #356)**: `train()` called `ctx.model.train()` unconditionally, so every `LockedDropout` site fired on both recal paths — and **14 of the 15 BatchNorms sit downstream of one** — while `ss_epsilon` was passed live into the `bn_recal_from` lesson loop. Two augmentation families were gated and two were not, by prose that claimed all of them. **C-328 instance 5**, and **C-303**. The fix puts every dropout module in `eval()` and forces `ss_epsilon=0.0` on a recal pass; it changes the running statistics written into FUTURE artifacts, which is the epic's only behavioural change. They recompute BatchNorm running statistics, which ship inside the artifact and are used at inference, so corrupting their inputs would put a treatment arm's BN layer on a different footing from its control for a reason that is not the hypothesis, and would accumulate half of every artifact's BN statistics on H/W-flipped fields that inference never produces (**C-328**). The flag is deliberately general rather than per-augmentation: C-328's own trigger is *adding an augmentation without asking which non-training forwards traverse it*, and the first fix for it named `apply_input_noise` and left the older flip in place. Pinned by `tests/train/test_bn_recal_augmentation.py`, which asserts no suppressed call flips and — as anti-vacuity — that ordinary calls do.
