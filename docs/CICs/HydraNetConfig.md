...
- **Field Validation:** Guarantees that all 100 fields are type-checked and constraint-validated (e.g., `dropout_rate` in [0.0, 1.0], `input_channels >= 1`, `rollout_horizon >= 1`). *(`ss_feedback` added 2026-07-27 — EXP-4/GTF bloom dossier: what scheduled sampling feeds back for a FAMILY head, `mean` (default, log1p E[y]) vs `sample` (composition-aware draw = train-exposure==deploy-exposure); validated in {mean, sample}; legacy point heads ignore it.)* *(`rollout_feedback` added 2026-07-26 — H-SAMPLE / EXP-2 bloom dossier: the autoregressive feedback copy, `mean` vs `sample` (ancestral) vs `teacher_forced` (oracle); HydraNetInference fails loud on a bad value or `sample` without a registered family. **Default changed 2026-07-27 (ADR-070 / Epic #193 S4): `None` = AUTO — resolves to `sample` for a registered family head (the C-113 bloom mitigation, T=0-neutral) and `mean` for a legacy head (byte-identical).** Field type `str | None`; count unchanged. **Verified (Epic #193 S7): sample bounds the bloom 9/9 arms×seeds; mean blooms 9/9.** T=0-neutrality is byte-exact at BOTH the distribution level (emit-mean/gate/params) AND the scored D×K cube — the latter via the per-`(pass,step)` sampler seeding in `to_cube_samples` (`66a95ea`), which isolates h=1's draws from the feedback-changed h≥2 params.)* *(`freeze_h` retired 2026-06-05; `rollout_horizon` added 2026-06-06 — Axis B / #78; `loss_reg_theta_init`, `learnable_theta`, `loss_class_pos_weight` added 2026-06-10 — hurdle-NB / D2 / #99; `output_distribution` added 2026-06-10 — hurdle-NB head activation / #100; `min_free_disk_gb` added 2026-06-13 — disk-headroom guard / C-154 / #107; `reg_activation` added 2026-06-21 — emit-activation decouple / Exp-B; `n_quantiles` + `loss_reg_tau`/`loss_reg_cap` added — quantile head / pinball body dial; `body_mask` added and `hurdle_threshold` retired 2026-07-18 — ADR-065 / Epic #158 (later retired, see below); `n_head_samples` added 2026-07-20 — ADR-067 / Epic #167 A-S5 (#172); `max_posterior_cube_gb` added 2026-07-21 — ADR-067 / Epic #167 A-S8 (#175); `pi_penalty_weight` + `pi_penalty_prior_logit` added 2026-07-21 — ADR-067 / Epic #167 A-S9 (#176) family π-ridge (C-200); `forecast_composition` + `gate_threshold` added 2026-07-24 — ADR-069 / Epic #183 S3 (#186), the composition axis (self_zeroed / soft_gate / threshold_gate + τ) enforcing the emit-time gate+body composition rules fail-loud, and the `output_distribution` allowed set is now the LEGACY values `{standard, hurdle_nb, hurdle_lognormal, hurdle_shrinkage, dense_nb, quantile}` **unioned with the registered distribution-family names** `family_names()` (currently `{nb, zinb, mixture_nb, truncated_nb}`), the two required disjoint (C-197). The classification head is reused as the hurdle gate `P(y>0)`; θ is the loss-owned Parameter in `TruncatedNBLoss`, not a model head. `body_mask` retired for `body_supervision ∈ {all, active}` + `onset_lead` + `cessation_lag` 2026-07-28 — ADR-065 amend.: the graded, asymmetric supervision window (net +2 fields → 88); resolved by `body_supervision.resolve_body_supervision`; validated by `validate_body_supervision` + `validate_body_supervision_latent` (C-193). See CIC `BodySupervisionResolver.md`.)* *(`emit_family_core` added — ADR-068 (th_gated_ZINBcore, Epic #167/#183): draw the π-stripped NB core and gate it externally; `bool`, default `False`; validated core-zinb-must-be-gated + rejected on nb (C-242). Net **+1 → 89**.)* *(`ss_reverse` added 2026-08-23 — #287 / Teutsch et al. 2022: INCREASING teacher forcing. `bool`, default `False`, which is **byte-identical** to the ADR-056 decreasing-TF curriculum — verified over every schedule, every warmup and all 300 lessons by `tests/test_scheduled_sampling_reverse.py::test_reverse_false_is_byte_identical_to_the_pre_flag_behaviour`, sabotage-verified. `True` starts ε at `ss_epsilon_max` and decays it to 0, so the model begins near free-running and is given progressively more ground truth; an ITF arm sets `ss_warmup_lessons` to the TOTAL lesson count because that is the linear ramp length. Net **+1 → 90**.)* *(`ss_backprop_through_feedback` added 2026-09-03 — #308 / `Vlachas2023_LearningFromPredictions` (BPTT-SA): whether the gradient may flow THROUGH the fed-back prediction during scheduled sampling. `bool`, default `False`, which leaves the `.detach()` in place and is byte-identical to the pre-flag behaviour. ⚠️ For a FAMILY head the fed value is a DRAW, which is not reparameterised — `d(draw)/d(params)` is exactly 0 — so `True` applies a **straight-through estimator**: forward is the draw (bit-identical, measured `max|diff| = 0`), backward is the *composed* mean's gradient. Simply un-detaching is a **no-op** and was shipped as one; see **C-324** and `reports/2026-09-03_bptt_sa_dossier/08_postmortem.md`. ⚠️ An earlier version of this note claimed the flag is *"guarded by a potency gate as a launch precondition"*. That is **not true in production**: `scripts/potency_check.py` has no caller in `views_hydranet/`, in any launcher, or in any model's `main.py` — the only invocation is a dossier tool that archives with its dossier. The gate is real and tested, but it is opted into per experiment, not enforced. **C-303.** ⚠️ **Second C-303 instance, fixed 2026-09-14 (Epic #353 / S2 #355):** `training_engine.py`'s `_attach_feedback_grad_clip` call site claimed *"A config validator rejects the combination outright"* for a legacy head with this flag set. No such validator existed — the flag validated on a point/quantile head, where `fed` IS the regression loss's input, the wire was connected across the full ~383-step graph, the per-step clip was silently skipped, and `fed_grad_max` logged a constant 0.0 that reads as a healthy gradient. Now enforced by `reject_bptt_sa_without_a_family`, mirroring `reject_pushforward_without_a_family`. Field count unchanged.)* *(`reject_pushforward_without_a_family` extended 2026-09-14 (#372 review): `pushforward_weight > 0` with `emit_family_core=True` on a self-zeroed family is rejected. The SS block already rejects that pair because `_family_feedback_log1p` is not core-aware, but that guard sits under `if ss_epsilon_max > 0`, which a pushforward arm never sets — the same nesting that hid S2's C-259 instance. Guarded for the pushforward independently.)* *(`ss_feedback_grad_clip` added 2026-09-04 — #308 GRAD-TRAJ follow-up: max L2 norm of the gradient leaving the scheduled-sampling handoff, applied **per step**. `float | None`, default `None` = no-op, so every existing arm is byte-identical; `gt=0` because a clip of 0 cuts the wire rather than bounding it and would be indistinguishable from the **C-324** no-op. **`clip_grad_norm` cannot substitute for it:** that runs once at the END of the backward pass, and GRAD-TRAJ measured the intermediate gradient rising to 9.4e9 — overflowing float32 at lesson 48 — with clipping on throughout. Acts only when `ss_backprop_through_feedback` is True; with the wire cut, `fed` is detached and carries no gradient to bound. Net **+1 → 95**.)* *(`input_noise_dropout` added 2026-09-04 — #311 / Epic rollout-robustness, `SanchezGonzalez2020_GraphNetworkSimulators` **adapted, not transplanted**: per-step probability that a surviving input event is silenced, accumulating within one deployment horizon (`time_steps`) and resetting between. `float | None`, default `None` = no-op, so every existing config is byte-identical; `gt=0.0` because a dropout of 0.0 is a no-op indistinguishable from off — the **C-324** inert-knob signature that cost 276 min of GPU on #308. **NOT the paper's Gaussian:** S1 (#313) measured this model's free-running error as near-total SILENCING (FN 0.9959 vs FP 0.000027 at h18, FN/FP = 36,870×), so dense additive noise would model a failure that does not occur — and on a 99.9%-zero `log1p` field it would produce off-manifold negative log-counts. Only removes, so it cannot manufacture occurrence (M45). Net **+1 → 96**.)* *(`reject_inert_clamp` added 2026-09-14 — Epic #353 / S1 #354: `freeze_recurrent` set with `freeze_recurrent_weight=0.0` is rejected. `blend_recurrent_state` at weight 0 returns the free-running state unchanged, so the pair logged CLAMPED and delivered the control — the **C-324/C-331** signature, promoted to production by ADR-027 §2.1. The field range stays `[0, 1]` inclusive per the ADR's Beige Team contract; the inert case is the *pair*, so it is a cross-field validator. `HydraNetInference.__init__` refuses the same pair for drivers that bypass pydantic. No field added.)* *(`require_cuda` added 2026-09-19 — #377: `bool`, default `False` (byte-identical; the CPU banner still fires). `True` makes a CPU device a hard stop at the top of training, evaluation and forecasting, with the banner's diagnosis — which now distinguishes a CPU-only torch build, no visible driver, and a driver too old for this torch's CUDA build; the last is what trained `violet_visitor` on CPU for 6 h 46 m on 2026-09-16 under a banner blaming the hardware. Roster configs should set it. Net **+1 → 100**.)* *(`freeze_recurrent` + `freeze_recurrent_weight` added 2026-09-05 — **ADR-027 §2.1**: the cell clamp promoted from a diagnostic constructor argument to a production setting. `str | None`, default `None` = the ADR-027 §2 behaviour (the full ConvLSTM state evolves), **byte-identical for every config that omits the key** — the property the amendment rests on, guarded by `tests/test_freeze_recurrent_config.py`. Validated in `{None, hidden, cell, all}` by `reject_unknown_freeze_recurrent_mode`, so a typo fails loud instead of silently leaving the rollout unclamped (the **C-324** inert-knob signature). **`cell` is the evidenced production value** — M48/M56 ΔAP@h36 **+0.0591**, 4/4 seeds, re-verified from raw artifacts as **M65** (+0.0367 at h18, +0.0000 at h1 on all four seeds); `hidden` has no consistent effect (M58) and `all` is not evidenced, so both stay diagnostics. **This does NOT reinstate `freeze_h`**, retired 2026-06-05 and still retired — that was a different mechanism answering a different question (it was inert against the C-113 *runaway*; this improves rollout *skill*). ⚠️ **Product change to disclose:** under the clamp the dispersion of predicted per-cell change falls −0.4563 (h18) and −0.6593 (h36), 4/4 seeds, while direction skill is *slightly better* at h18 and `crps_events` improves 4/4 — equally good at **where**, flatter about **how much** (M58). `freeze_recurrent_weight` is a field, not a constant (**C-85**), but the dial is **CLOSED**. ⚠️ **CORRECTED 2026-09-06:** this entry previously claimed M41's saturation at w≈0.1 was measured on the 40-lesson vehicle and never re-tested at L=300. **That was false.** M41 **is** at L=300 (`w=0` → 0.3318, the L=300 reference), seed 43, re-verified on the corrected yardstick 2026-08-22, and it **closes** the dial rather than leaving it open: the paired interval for `cell@0.5` vs `cell` (MDE 0.0045) includes zero at h18 and h36, and **at h6 the hard clamp is significantly better** ([−0.0107, −0.0011]). `1.0` is the default on evidence, not by default. **C-303**, found by a method-review seat reading the ledger. Net **+2 → 98**.)* *(`bn_recalibrate` added 2026-09-06 — the **C-184 mitigation** (post-training BatchNorm recalibration, whose buffers ship inside every artifact) had been read as `config.get("bn_recalibrate", True)`: a **shadow default on a production setting that no schema validated**. All 8 roster configs set it, and a typo would have silently reverted to `True` with nothing raised. `bool`, default `True` — byte-identical for every existing config. Net **+1 → 99**. Shipped with `reject_near_miss_keys`, which splits a typo from another layer's key: `extra="allow"` stays (views-pipeline-core legitimately reads `skip_predictions_delivery` from these configs), but an unknown key within **one edit** of a real field now fails loud — the **C-324** inert-knob signature, and the reason `freeze_recurent: 'cell'` would otherwise validate, do nothing, and look enabled.)*
...
- **Conditional Parameter Validation:** Guarantees that strategy-specific parameters are explicitly provided for the active choice in `sampling_strategy`, `loss_reg`, `loss_class`, `body_supervision` (QS99 params), and `target_weights` (regression target coverage). No silent defaults — missing parameters raise immediately.
...
- **Construction:** Assumes keyword arguments matching the 100 defined fields. Extra fields are tolerated (`extra = "allow"`). *(Said **64** until 2026-09-14 (Epic #353 / S10 #363) — stale since long before the count reached 99, and contradicting this same document's own §Field Validation line. Verified against `len(HydraNetConfig.model_fields)`, not against the other paragraph.)*
...
- **Retired Hurdle Knobs (Epic #158 / ADR-065):** Raises `ValueError` when a retired knob (`hurdle_threshold`, `hurdle_mask_mode`) is set — these migrated to the `body_supervision` axis; a config still setting them fails loud with a migration message (`reject_retired_hurdle_knobs`).
- **Missing QS99 Tau (ADR-050):** Raises `ValueError` when `body_supervision='active'` and `qs99_weight > 0` but `qs99_tau` is not provided (`validate_qs99_requires_tau` — re-keyed off the retired `hurdle_threshold` onto `body_supervision`).
...
- **Unwired rollout_horizon (C-264):** Raises `ValueError` when `rollout_horizon != 1` — the ADR-058 B1 pushforward consumer is not wired, so `K>1` would silently train one-step (`reject_unwired_rollout_horizon`). Default `1` passes.
- **Scheduled-Sampling Feedback Mismatch (ADR-056 / C-259):** when `ss_epsilon_max > 0`, raises `ValueError` unless `ss_feedback` equals the resolved `rollout_feedback` (`None`→`'sample'` for a family head), and rejects a gated `ss_feedback='mean'` — training must feed back the same object inference rolls out on.
- **Scheduled-Sampling Channel Order (C-260):** when `ss_epsilon_max > 0`, raises `ValueError` unless `features == regression_targets` in ORDER — the AR substitution feeds the target forecasts into the feature channels positionally, so a reordered/extended config would silently cross-wire targets.
