...
**Status:** Rejected as a C-113 *fix* (falsified — see Outcome) · the locked-mask *mechanism* is **Accepted as the inference posterior default** per **[ADR-059](../active/059_predictive_uncertainty_representation.md)** (shipped on `development`, 2026-06-06).
**Date:** 2026-06-03
...
> **Reconciliation (2026-06-06, ADR-059):** the `LockedDropout` mechanism this ADR defines **did ship** — it is the production MC-dropout posterior (inference-only, magnitude-neutral). What was falsified was its role as a *C-113 stabiliser*, **not** the mechanism itself. The *which-posterior* decision (dropout now → learned posterior target) is recorded in **ADR-059**; this ADR remains the record of the *locked-mask mechanism* (locked vs per-step masks). So: ✗ as a C-113 fix, ✓ as the posterior-sampling mechanism.
**Branch:** merged to `development` via PR #75 (2026-06-06); the `fix/variational-dropout-autoregressive-stability` branch was deleted post-merge.
