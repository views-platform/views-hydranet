...
We implement a mandatory, persistent pattern for all `raise` statements in the HydraNet codebase. Every structural failure must follow a three-step sequence to satisfy the **Double-Lock Protocol**.

### 2.1 The Three-Step Sequence
1.  **Message Assignment:** Assign the descriptive error message to a local variable named `err_msg`.
2.  **Explicit Logging:** Call `logger.error(err_msg)` or `logger.critical(err_msg)` before raising.
...
# CANONICAL PATTERN (CRITICAL)
err_msg = "Unrecoverable system state corruption detected."
